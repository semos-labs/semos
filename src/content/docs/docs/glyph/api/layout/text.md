...
| <a id="focusedstyle"></a> `focusedStyle?` | [`Style`](../../core/style) | Style applied when this element is focused (merged with `style`). |
| <a id="style"></a> `style?` | [`Style`](../../core/style) | Text style (color, bold, dim, italic, underline, etc.). |
| <a id="wrap"></a> `wrap?` | [`Responsive`](../../core/style#responsive)\<[`WrapMode`](../../core/style#wrap-mode)\> | Text wrapping mode. Overrides `style.wrap`. - `"wrap"` — soft-wrap at the container edge (default) - `"truncate"` — cut off with no indicator - `"ellipsis"` — cut off with `…` - `"none"` — no wrapping at all |
